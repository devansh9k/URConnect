  ...
  return (
    <Fade bottom>
      ...
      >
        ...
        <PostBody>
          <Post__question>
            <Question__Field onClick={handleShowAnswer}>
              <div>
                <h4>{question}</h4>
              </div>
          ...
          <Post__answer show={showAnswer}>
            {allanswer &&
              allanswer.map(({ id, Answers }) => (
                <Post__Answer__q key={id}>
...
const Post__Answer__q = styled.div`
  display: flex;
  flex-direction: row;
  align-items: center;
`;
...
const Answer__Wrapper = styled.div`
  display: flex;
  flex-direction: row;
